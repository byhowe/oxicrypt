...
//! let ikm = [11; 22];
//! let prk = HmacSha256::oneshot(i, salt, ikm);
//! ```
...
pub struct Hkdf<D, const M: usize, const O: usize, const B: usize>
where
...
impl<D, const M: usize, const O: usize, const B: usize> Hkdf<D, M, O, B>
where
  D: core::fmt::Debug + Clone + Copy,
{
  pub fn with_prk<K: AsRef<[u8]>>(implementation: Implementation, prk: K) -> Result<Self, LenError>
  {
    let prk = prk.as_ref();
    if prk.len() < O {
    ...
    unsafe { ctx.assume_init_mut().set_prk_unchecked(implementation, prk) };
    Ok(unsafe { ctx.assume_init() })
  }

  pub unsafe fn with_prk_unchecked<K: AsRef<[u8]>>(implementation: Implementation, prk: K) -> Self
  {
    let mut ctx: MaybeUninit<Self> = MaybeUninit::uninit();
    ctx.assume_init_mut().set_prk_unchecked(implementation, prk);
    ctx.assume_init()
  }

  pub fn set_prk<K: AsRef<[u8]>>(&mut self, implementation: Implementation, prk: K) -> Result<(), LenError>
  {
    let prk = prk.as_ref();
    if prk.len() < O {
      return Err(LenError::Prk {
    ...
    unsafe { self.set_prk_unchecked(implementation, prk) };
    Ok(())
  }

  pub unsafe fn set_prk_unchecked<K: AsRef<[u8]>>(&mut self, implementation: Implementation, prk: K)
  {
    self.hmac.set_key(implementation, prk);
  }

  pub fn expand(&self, implementation: Implementation, info: &[&[u8]], okm: &mut [u8]) -> Result<(), LenError>
  {
    if okm.len() > 255 * O {
    ...
    unsafe { self.expand_unchecked(implementation, info, okm) };
    Ok(())
  }

  pub unsafe fn expand_unchecked(&self, implementation: Implementation, info: &[&[u8]], okm: &mut [u8])
  {
    let mut t: &[u8] = b"";
    let mut i = 0x00;
    ...
    for chunk in okm.chunks_mut(O) {
      i += 1;
      hmac = self.hmac;
      hmac.update(implementation, t);
      info.iter().for_each(|i| hmac.update(implementation, i));
      hmac.update(implementation, &[i]);
      hmac.finish_into(implementation, chunk);
      t = chunk;
    }
  }
}
