macro_rules! sha2_32_f {
    ($a:ident, $b:ident, $c:ident, $d:ident, $e:ident, $f:ident, $g:ident, $h:ident, $m1:ident, $m2:ident, $m3:ident, $m4:ident, $magic:expr) => {{
        ...
        $h = $h
            .wrapping_add(a_rho)
            .wrapping_add(($a & $b) | (($a | $b) & $c));
        $m1 = $m1
            .wrapping_add(m2_sigma)
            .wrapping_add($m3)
            .wrapping_add(m4_sigma);
    }};
}

/// Compression function used by the SHA-2 family of functions, namely SHA-224
/// and SHA-256.
///
...
/// # Safety
///
/// The caller must guarantee that the passed variables point to valid memory
/// spaces. `state` must point to an array with a length of 8 (32 bytes).
/// `block` must point to an array with a length of 64 (64 bytes).
#[allow(clippy::many_single_char_names)]
#[allow(unused_assignments)]
pub const unsafe fn sha256(state: *mut u32, block: *const u8)
{
