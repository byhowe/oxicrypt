...
/// AES comes in three variants. This enum is used to represent which one to use.
...
/// decrypting the data. This type does not keep track of wheter it was created for encryption or
  ...
  [(); V.key_sched_len()]:,
{
  ...
  ///
  /// Returns an [`Err`](`Result::Err`) when length of the `key` is not equal to
  /// 16 for AES128, 24 for AES192, 32 for AES256.
  pub fn with_encrypt_key(key: &[u8]) -> Result<Self, LenError>
  {
    ...
    unsafe { key_schedule.assume_init_mut().set_encrypt_key_unchecked(key) };
  ...
  /// # Safety
  ///
  /// * Length of `key` must be equal to 16 for AES128, 24 for AES192, 32 for AES256.
  pub unsafe fn with_encrypt_key_unchecked(key: &[u8]) -> Result<Self, LenError>
  {
    let mut key_schedule: MaybeUninit<Self> = MaybeUninit::uninit();
    key_schedule.assume_init_mut().set_encrypt_key_unchecked(key);
    Ok(key_schedule.assume_init())
  }

  /// Creates a key schedule to use in decryption mode.
  ///
  /// Returns an [`Err`](`Result::Err`) when length of the `key` is not equal to 16 for AES128, 24
  /// for AES192, 32 for AES256.
  pub fn with_decrypt_key(key: &[u8]) -> Result<Self, LenError>
  {
    ...
    let mut key_schedule: MaybeUninit<Self> = MaybeUninit::uninit();
    unsafe { key_schedule.assume_init_mut().set_decrypt_key_unchecked(key) };
    Ok(unsafe { key_schedule.assume_init() })
  }
  ...
  /// # Safety
  ///
  /// * Length of the `key` must be equal to 16 for AES128, 24 for AES192, 32 for AES256.
  pub unsafe fn with_decrypt_key_unchecked(key: &[u8]) -> Result<Self, LenError>
  {
    let mut key_schedule: MaybeUninit<Self> = MaybeUninit::uninit();
    key_schedule.assume_init_mut().set_decrypt_key_unchecked(key);
    Ok(key_schedule.assume_init())
  }
  ...
  pub unsafe fn set_encrypt_key_unchecked(&mut self, key: &[u8])
  {
    if is_x86_feature_detected!("aes") {
      match V {
        | Aes128 => unsafe { aes_x86_aesni_aes128_expand_key(key.as_ptr(), self.as_mut_ptr()) },
        | Aes192 => unsafe { aes_x86_aesni_aes192_expand_key(key.as_ptr(), self.as_mut_ptr()) },
        | Aes256 => unsafe { aes_x86_aesni_aes256_expand_key(key.as_ptr(), self.as_mut_ptr()) },
      }
    } else {
      match V {
  ...
  ///
  /// Returns an [`Err`](`Result::Err`) when the length of `block` is not a multiple of 16.
    ...
    if is_x86_feature_detected!("aes") {
      while !block.is_empty() {
        match block.len() / 16 {
          | n if n >= 8 => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_encrypt8(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_encrypt8(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_encrypt8(block.as_mut_ptr(), self.as_ptr()),
            }
            block = &mut block[8 * 16 ..];
          }
          | n if n >= 4 => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_encrypt4(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_encrypt4(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_encrypt4(block.as_mut_ptr(), self.as_ptr()),
            }
            block = &mut block[4 * 16 ..];
          }
          | n if n >= 2 => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_encrypt2(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_encrypt2(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_encrypt2(block.as_mut_ptr(), self.as_ptr()),
            }
            block = &mut block[2 * 16 ..];
          }
          | _ => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_encrypt1(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_encrypt1(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_encrypt1(block.as_mut_ptr(), self.as_ptr()),
            }
    ...
    if is_x86_feature_detected!("aes") {
      while !block.is_empty() {
        match block.len() / 16 {
          | n if n >= 8 => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_decrypt8(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_decrypt8(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_decrypt8(block.as_mut_ptr(), self.as_ptr()),
            }
            block = &mut block[8 * 16 ..];
          }
          | n if n >= 4 => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_decrypt4(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_decrypt4(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_decrypt4(block.as_mut_ptr(), self.as_ptr()),
            }
            block = &mut block[4 * 16 ..];
          }
          | n if n >= 2 => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_decrypt2(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_decrypt2(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_decrypt2(block.as_mut_ptr(), self.as_ptr()),
            }
            block = &mut block[2 * 16 ..];
          }
          | _ => {
            match V {
              | Aes128 => aes_x86_aesni_aes128_decrypt1(block.as_mut_ptr(), self.as_ptr()),
              | Aes192 => aes_x86_aesni_aes192_decrypt1(block.as_mut_ptr(), self.as_ptr()),
              | Aes256 => aes_x86_aesni_aes256_decrypt1(block.as_mut_ptr(), self.as_ptr()),
            }
            block = &mut block[1 * 16 ..];
          }
        }
