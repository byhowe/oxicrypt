...
///
/// After this process, the block will have an empty space at the end worth
/// `bit_count_len` bytes. This space should be filled with the bit count
/// information. The bit count information should be calculated before the call
/// to this `pad` functions since the function modifes the value of `index`. The
/// endiannes of the bit count also matters. SHA-1 and SHA-2 use big-endian
/// while MD5 uses little endian. The process of length-padding is caleld
/// Merkle–Damgård strengthening. After this strengthening process, call the
/// appropriate compress function once again and change the endiannes of the
/// state integers according to what the algorithm calls for.
pub fn pad<D: DigestMeta, C: Fn()>(
    block: &mut [u8],
    index: &mut usize,
    bit_count_len: usize,
    compress: C,
