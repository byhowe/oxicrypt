...
#define sha_finish(maxblocklen, digestlen, lenlen, statety, statetobe,         \
    compress)                                                                  \
  ctx->block[ctx->blocklen] = 0b10000000;                                      \
  ctx->len += ctx->blocklen;                                                   \
  ctx->blocklen += 1;                                                          \
                                                                               \
  if (ctx->blocklen > (maxblocklen) - (lenlen)) {                              \
    memset(ctx->block + ctx->blocklen, 0, maxblocklen - ctx->blocklen);        \
    (compress)(ctx->h, ctx->block);                                            \
    ctx->blocklen = 0;                                                         \
  }                                                                            \
                                                                               \
  memset(ctx->block + ctx->blocklen, 0, maxblocklen - 8 - ctx->blocklen);      \
  ctx->len *= 8;                                                               \
  ctx->len = htobe64(ctx->len);                                                \
  memcpy(ctx->block + maxblocklen - 8, &ctx->len, 8);                          \
  (compress)(ctx->h, ctx->block);                                              \
                                                                               \
  statety cur;                                                                 \
  for (uint8_t i = 0; i < (outlen > digestlen ? digestlen : outlen); i += 1) { \
    const size_t cur_bytes_rem = i % sizeof(statety);                          \
    if (cur_bytes_rem == 0) {                                                  \
      cur = statetobe(ctx->h[i / sizeof(statety)]);                            \
    }                                                                          \
    out[i] = ((uint8_t*)&cur)[cur_bytes_rem];                                  \
  }

void oxi_sha1_finish(oxi_sha1_ctx_t* ctx, uint8_t* out, size_t outlen)
{
  sha_finish(OXI_SHA1_BLOCK_LEN, OXI_SHA1_DIGEST_LEN, 8, uint32_t, htobe32,
      oxi_sha1_compress_generic);
}

void oxi_sha224_finish(oxi_sha224_ctx_t* ctx, uint8_t* out,
    size_t outlen)
{
  sha_finish(OXI_SHA224_BLOCK_LEN, OXI_SHA224_DIGEST_LEN, 8, uint32_t, htobe32,
      oxi_sha256_compress_generic);
}

void oxi_sha256_finish(oxi_sha256_ctx_t* ctx, uint8_t* out,
    size_t outlen)
{
  sha_finish(OXI_SHA256_BLOCK_LEN, OXI_SHA256_DIGEST_LEN, 8, uint32_t, htobe32,
      oxi_sha256_compress_generic);
}

void oxi_sha384_finish(oxi_sha384_ctx_t* ctx, uint8_t* out,
    size_t outlen)
{
  sha_finish(OXI_SHA384_BLOCK_LEN, OXI_SHA384_DIGEST_LEN, 16, uint64_t, htobe64,
      oxi_sha512_compress_generic);
}

void oxi_sha512_finish(oxi_sha512_ctx_t* ctx, uint8_t* out,
    size_t outlen)
{
  sha_finish(OXI_SHA512_BLOCK_LEN, OXI_SHA512_DIGEST_LEN, 16, uint64_t, htobe64,
      oxi_sha512_compress_generic);
}

void oxi_sha512_224_finish(oxi_sha512_224_ctx_t* ctx, uint8_t* out,
    size_t outlen)
{
  sha_finish(OXI_SHA512_224_BLOCK_LEN, OXI_SHA512_224_DIGEST_LEN, 16, uint64_t,
      htobe64, oxi_sha512_compress_generic);
}

void oxi_sha512_256_finish(oxi_sha512_256_ctx_t* ctx, uint8_t* out,
    size_t outlen)
{
  sha_finish(OXI_SHA512_256_BLOCK_LEN, OXI_SHA512_256_DIGEST_LEN, 16, uint64_t,
      htobe64, oxi_sha512_compress_generic);
}
