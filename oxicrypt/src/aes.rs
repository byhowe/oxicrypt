...
pub fn aes_encrypt(data: &[u8], key: &[u8])
{
  assert!(data.len() >= 16);
  assert!(key.len() >= 16);
  ...
  let number_of_rounds: u32 = 1;

  key_expansion();
  add_round_key(&mut state, &mut key);
...
fn key_expansion_core(input: &mut [u8], i: u8) {
...
fn add_round_key(state: &mut [u8], round_key: &mut [u8])
{
  for i in 0 .. 16 {
    state[i] ^= round_key[i];
  }
