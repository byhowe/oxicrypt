...
pub fn criterion_benchmark(c: &mut Criterion)
{
        ...
        b.iter(|| unsafe {
            for i in 0..16 {
                aes_x86_aesni_aes128_encrypt1(
                    black_box(block.as_mut_ptr().add(i * 1 * 16)),
    ...
    let mut block = B;
    group.bench_function("aes 128 2", |b| {
        b.iter(|| unsafe {
            for i in 0..8 {
                aes_x86_aesni_aes128_encrypt2(
                    black_box(block.as_mut_ptr().add(i * 2 * 16)),
                    black_box(keysched.as_ptr()),
                );
            }
        })
    ...
    let mut block = B;
    group.bench_function("aes 128 4", |b| {
        b.iter(|| unsafe {
            for i in 0..4 {
                aes_x86_aesni_aes128_encrypt4(
                    black_box(block.as_mut_ptr().add(i * 4 * 16)),
                    black_box(keysched.as_ptr()),
                );
            }
        })
        ...
        b.iter(|| unsafe {
            for i in 0..2 {
                aes_x86_aesni_aes128_encrypt8(
                    black_box(block.as_mut_ptr().add(i * 8 * 16)),
                    black_box(keysched.as_ptr()),
                );
            }
        })
