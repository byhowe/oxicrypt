...
#[cfg(feature = "generate")] mod aesni_intel;
...
{
    pub fn new(buffer: &'a [u8]) -> Self { Self { buffer, index: 0 } }
...
{
    pub fn new(buffer: &'a mut [u8]) -> Self { Self { buffer, index: 0 } }
    ...
    pub fn n_written(&self) -> usize { self.index }
}
