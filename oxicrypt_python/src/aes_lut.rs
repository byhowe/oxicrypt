...
{
  oxicrypt_core::aes_lut_aes128_encrypt1(block.as_bytes_mut().as_mut_ptr(), key_schedule.as_bytes().as_ptr())
}
#[pyfunction]
unsafe fn aes_lut_aes192_encrypt1(block: &PyByteArray, key_schedule: &PyBytes)
{
  oxicrypt_core::aes_lut_aes192_encrypt1(block.as_bytes_mut().as_mut_ptr(), key_schedule.as_bytes().as_ptr())
}
#[pyfunction]
unsafe fn aes_lut_aes256_encrypt1(block: &PyByteArray, key_schedule: &PyBytes)
{
  oxicrypt_core::aes_lut_aes256_encrypt1(block.as_bytes_mut().as_mut_ptr(), key_schedule.as_bytes().as_ptr())
}

#[pyfunction]
unsafe fn aes_lut_aes128_decrypt1(block: &PyByteArray, key_schedule: &PyBytes)
{
  oxicrypt_core::aes_lut_aes128_decrypt1(block.as_bytes_mut().as_mut_ptr(), key_schedule.as_bytes().as_ptr())
}
#[pyfunction]
unsafe fn aes_lut_aes192_decrypt1(block: &PyByteArray, key_schedule: &PyBytes)
{
  oxicrypt_core::aes_lut_aes192_decrypt1(block.as_bytes_mut().as_mut_ptr(), key_schedule.as_bytes().as_ptr())
}
#[pyfunction]
unsafe fn aes_lut_aes256_decrypt1(block: &PyByteArray, key_schedule: &PyBytes)
{
  oxicrypt_core::aes_lut_aes256_decrypt1(block.as_bytes_mut().as_mut_ptr(), key_schedule.as_bytes().as_ptr())
}
...
{
  oxicrypt_core::aes_lut_aes128_expand_key(key.as_bytes().as_ptr(), key_schedule.as_bytes_mut().as_mut_ptr())
}
#[pyfunction]
unsafe fn aes_lut_aes192_expand_key(key: &PyBytes, key_schedule: &PyByteArray)
{
  oxicrypt_core::aes_lut_aes192_expand_key(key.as_bytes().as_ptr(), key_schedule.as_bytes_mut().as_mut_ptr())
}
#[pyfunction]
unsafe fn aes_lut_aes256_expand_key(key: &PyBytes, key_schedule: &PyByteArray)
{
  oxicrypt_core::aes_lut_aes256_expand_key(key.as_bytes().as_ptr(), key_schedule.as_bytes_mut().as_mut_ptr())
}
